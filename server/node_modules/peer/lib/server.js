    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, function(err) {
        if (!err) {
          ...
          self._configureWS(socket, key, id, token, domain);
        } else {
...
app._configureWS = function(socket, key, id, token, domain) {
  ...
  socket.on('message', function(data) {
    try {
      ...
      if(message.type === 'HOST_CLAIM') {
         self.emit('host-claim', id, domain, socket);
         return;
      }
  ...
  // disconnects.
  this.emit('connection', id, domain, socket);
};
